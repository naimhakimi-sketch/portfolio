...
<div class="content">
    <form method="POST" action="{{ route('admin.work.update', $work) }}" enctype="multipart/form-data">
        ...
        <div class="form-group @error('position') error @enderror">
            ...
            <input type="text" id="position" name="position" value="{{ old('position', $work->position) }}" placeholder="e.g., Senior Web Developer" required>
            @error('position') <div class="error-message">{{ $message }}</div> @enderror
        </div>

        <div class="form-group @error('company') error @enderror">
            <label for="company">Company</label>
            <input type="text" id="company" name="company" value="{{ old('company', $work->company) }}" placeholder="Company Name" required>
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem;">
            <div class="form-group @error('year_start') error @enderror">
                ...
                <input type="text" id="year_start" name="year_start" value="{{ old('year_start', $work->year_start) }}" placeholder="2020" required>
            ...
            <div class="form-group @error('year_end') error @enderror">
                ...
                <input type="text" id="year_end" name="year_end" value="{{ old('year_end', $work->year_end) }}" placeholder="2024" required>
        ...
        <div class="form-group @error('description') error @enderror">
            ...
            <textarea id="description" name="description" required>{{ old('description', $work->description) }}</textarea>
        ...
        <div class="form-group @error('logo') error @enderror">
            ...
            @if($work->logo)
                <p style="margin-top: 0.5rem; color: #666;">
                    Current: <img src="{{ Storage::url($work->logo) }}" alt="Logo" style="max-width: 100px; max-height: 100px;">
                </p>
            @endif
            @error('logo') <div class="error-message">{{ $message }}</div> @enderror
        </div>

        <button type="submit" class="btn btn-success">Update Work Experience</button>
