...
<div class="content">
    <form method="POST" action="{{ route('admin.education.update', $education) }}" enctype="multipart/form-data">
        ...
        <div class="form-group @error('title') error @enderror">
            ...
            <input type="text" id="title" name="title" value="{{ old('title', $education->title) }}" placeholder="e.g., Bachelor of Science in Computer Science" required>
            @error('title') <div class="error-message">{{ $message }}</div> @enderror
        </div>

        <div class="form-group @error('institution') error @enderror">
            <label for="institution">Institution</label>
            <input type="text" id="institution" name="institution" value="{{ old('institution', $education->institution) }}" placeholder="University or College Name" required>
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem;">
            <div class="form-group @error('year_start') error @enderror">
                ...
                <input type="text" id="year_start" name="year_start" value="{{ old('year_start', $education->year_start) }}" placeholder="2020" required>
            ...
            <div class="form-group @error('year_end') error @enderror">
                ...
                <input type="text" id="year_end" name="year_end" value="{{ old('year_end', $education->year_end) }}" placeholder="2024" required>
        ...
        <div class="form-group @error('grade') error @enderror">
            ...
            <input type="text" id="grade" name="grade" value="{{ old('grade', $education->grade) }}" placeholder="e.g., 3.8 GPA, A+">
            @error('grade') <div class="error-message">{{ $message }}</div> @enderror
        </div>

        <div class="form-group @error('description') error @enderror">
            <label for="description">Description</label>
            <textarea id="description" name="description" required>{{ old('description', $education->description) }}</textarea>
        ...
        <div class="form-group @error('logo') error @enderror">
            ...
            @if($education->logo)
                <p style="margin-top: 0.5rem; color: #666;">
                    Current: <img src="{{ Storage::url($education->logo) }}" alt="Logo" style="max-width: 100px; max-height: 100px;">
                </p>
            @endif
